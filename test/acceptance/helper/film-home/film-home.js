...
module.exports = function FilmHome(){
    ...
    this.btnReelShare = element.all(by.xpath('//div[@ng-click="openShareModal($event, reel)"]')).first();
    this.shareOtherUsersOption =$('#share-other-users-cta');
    this.userSearchBox = element(by.model('filter.fullName'));
    this.userSearchResult = element.all(by.repeater('match in matches')).first();
    ...
    this.sharedTeam = element(by.xpath('//strong[@ng-if="share.sharedWithTeamId"]'));
    ...
    /*
    this.share = function(userName){
        var self = this;

        return self.waitForClickable(userSearchBox).sendKeys(userName).then(()=>{
            return self.waitForClickable(userSearchResult).then(()=>{
                return self.waitForClickable(btnAdd);
            })
        })
    }
    */
