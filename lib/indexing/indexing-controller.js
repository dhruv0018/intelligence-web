...
Indexing.controller('indexing.Controller', [
    '$window', '$rootScope', '$scope', '$sce', '$state', '$stateParams', '$modal', 'VIDEO_STATUSES', 'VG_EVENTS', 'AlertsService', 'IndexingService', 'PlaysFactory', 'PlayersFactory', 'indexing.Sidebar',
    function controller($window, $rootScope, $scope, $sce, $state, $stateParams, $modal, VIDEO_STATUSES, VG_EVENTS, alerts, indexing, plays, players, sidebar) {
        ...
        Mousetrap.bind('esc', function(event) {
            ...
            event.preventDefault();
            if (self.state === 'INDEXING') self.back();
            return false;
        });
        ...
        this.index = function() {
            ...
            if (self.state) self.state = 'INDEXING';
        };
