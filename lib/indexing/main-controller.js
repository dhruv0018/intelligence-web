...
Indexing.controller('Indexing.Main.Controller', [
    ...
    function controller(config, $window, $rootScope, $scope, $sce, $state, $stateParams, $modal, VIDEO_STATUSES, GAME_STATUSES, VG_EVENTS, alerts, indexing, plays, players, tags, play, event, sidebar) {
        ...
        $scope.sources = [
            {src: $sce.trustAsResourceUrl('http://localhost:8000/intelligence/assets/small.mp4'), type: 'video/mp4'}
        ...
        this.nextable = function() {
            ...
            return $scope.formIndexing.$dirty &&
                   $scope.formIndexing.$valid &&
                   event.allVariablesHaveValues();
        };
        ...
        this.back = function() {
            ...
            else if (event.activeVariableIndex === 1 &&
                    !event.activeVariableValue()) {
            ...
            else if (!event.activeVariableValue()) {

                /* Move back one variable. */
                --event.activeVariableIndex;

                /* Clear the variable before. */
                event.clearActiveVariableValue();
            }
