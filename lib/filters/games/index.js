...
function gameIsReadyForQa (
    ...
    return (games) => games.filter(function(game) {
        return game.status === GAME_STATUSES.READY_FOR_QA.id;
    });
}
 ...
 * Games filter.
 * @module Games
 * @name gameNotIndexedByMe
 * @type {Filter}
 */

gameNotIndexedByMe.$inject = [
    'SessionService'
];

function gameNotIndexedByMe (
    session
) {
    return (games) => games.filter(function(game) {
        ...
        if(!lastIndexerAssignment.isQa && lastIndexerAssignment.userId !== currentUser) {
            return true;
        }
 ...
 * Games filter.
 * @module Games
 * @name gameHasNotExpired
 * @type {Filter}
 */

gameHasNotExpired.$inject = [
    'SessionService',
    'TeamsFactory'
];

function gameHasNotExpired (
    session,
    teams
) {
    return (games, flag) => games.filter(function(game) {
        let submittedAt = moment.utc(game.submittedAt);
        ...
        return submittedAt.add(turnaroundTime).isBefore(now);
    });
}
...
Games.filter('gameNotIndexedByMe', gameNotIndexedByMe);
Games.filter('gameHasNotExpired', gameHasNotExpired);
