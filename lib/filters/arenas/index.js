...
function ArenaEventsFilter() {
    ...
    return function arenaEventsFilter(arenaEvents, query) {
         ...
         * filtering rules. If the arena event has passed all of the rules
         * based on current query conditions, than the arena event will be
         * included in the filteredArenaEvents */

        const filteredArenaEvents = arenaEvents.filter((arenaEvent) => {
            ...
            if (specificPeriodFilter &&
                !isArenaEventInPeriodFilter(arenaEvent, query.period)) return false;
...
function isArenaEventInPeriodFilter(arenaEvent, periodsQuery) {

    if (arenaEvent.period === '1' && periodsQuery.one) {
        return true;
    } else if (arenaEvent.period === '2' && periodsQuery.two) {
        return true;
    } else if (arenaEvent.period === '3' && periodsQuery.three) {
        return true;
    } else if (arenaEvent.period === '4' && periodsQuery.four) {
        return true;
    } else if (arenaEvent.period === 'OT' && periodsQuery.overtime) {
        return true;
    } else {
        return false;
    }
