...
Users.directive('krossoverNewRole', [
    'ROLES', 'ROLE_TYPE',
    function directive(ROLES, ROLE_TYPE) {
...
Users.controller('UserController', [
    '$rootScope', '$scope', '$state', 'SessionService', 'ROLES', 'ROLE_TYPE', 'UserValue', 'UsersFactory',
    function controller($rootScope, $scope, $state, session, ROLES, ROLE_TYPE, user, users) {

        $scope.session = session;
        $scope.currentRole = session.currentUser.currentRole;

        $scope.$watch('session', function(newSession) {

            if (newSession) $scope.currentRole = newSession.currentUser.currentRole;
        ...
        var firstRole = $scope.user.roles ? $scope.user.roles[0] : undefined;
        var defaultRole = $scope.user.getDefaultRole();
        var newUserRole = {
            type: {
                id: '0',
                name: 'New User'
        ...
        $scope.role = defaultRole || firstRole || newUserRole;
        ...
        $scope.addNewRole = function(newRole) {
            ...
            if (newRole.type.id === ROLE_TYPE.ADMIN) {
