...
Account.controller('AccountController', [
    ...
    function controller($scope, session, ROLES, ROLE_ID) {
        ...
        for (var i = 1; i <= Object.keys(ROLE_ID).length; i += 1) {
            var roleGroup = {
                roleName: ROLES[ROLE_ID[i]].type.name,
                userRoles: session.currentUser.roles.filter(roleFilter)
            };
            if (roleGroup.userRoles.length > 0) {
