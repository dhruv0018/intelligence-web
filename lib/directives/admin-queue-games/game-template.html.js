...
<td>
    <span ng-if="(game.status ===  GAME_STATUSES.NOT_INDEXED.id ||
        game.status ===  GAME_STATUSES.SUBMITTED_FOR_INDEXING.id) &&
        game.indexerAssignments.length === 0">
        Can't be assigned yet
    </span>

    <span ng-if="!game.getStatus() ||
        (game.status === GAME_STATUSES.FINALIZED.id || game.status === GAME_STATUSES.INDEXED.id) &&
        game.indexerAssignments.length === 0">
        N/A
    </span>
...
<td>
    ...
    <span ng-if="game.status && (game.indexerAssignments.length === 0 ||
                (game.indexerAssignments.length > 0 && !game.hasQaAssignment() && !game.canBeAssignedToQa()))">
        Can't be assigned yet
    </span>

    <span ng-if="!game.status || (game.status === GAME_STATUSES.FINALIZED.id ||
                game.status === GAME_STATUSES.INDEXED.id) &&
                game.indexerAssignments.length === 0">
                N/A
    </span>
