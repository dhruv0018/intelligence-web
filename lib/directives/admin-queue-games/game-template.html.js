...
<td>{{ sports[leagues[teams[game.uploaderTeamId].leagueId].sportId].name | capitalizeFirstLetter }}</td>


<td class="game-id">{{ game.id }}</td>

<td>
    <a data-ui-sref="game({ id: game.id })">
        <span ng-if="game.teamId"> {{ teams[game.teamId].name }}</span>
        <span ng-if="!game.teamId"> Not Provided </span>
        vs
        <span ng-if="game.opposingTeamId"> {{ teams[game.opposingTeamId].name }} </span>
        <span ng-if="!game.opposingTeamId"> Not Provided </span>
    </a>
</td>
...
<td>
    <span ng-if="!game.currentBreakdownDistribution">--</span>
    <span ng-if="game.currentBreakdownDistribution">{{game.currentBreakdownDistribution.indexerGroup.name}}</span>
</td>
...
<td class="time-left">
    <span
        class="late"
        ng-if="timeRemaining(game) < 0"
        refresh-on="gamesChanged"
    >{{ timeRemaining(game) | millisecondsAsHours | hoursAsClock }}</span>
    <span
        ng-if="timeRemaining(game) === 0"
        refresh-on="gamesChanged"
    >None</span>
