...
module.exports = [
    '$state', 'TelestrationInterface', '$window', 'EventEmitter', 'EVENT_MAP', 'GlyphFactory', 'PlayManager', 'TELESTRATIONS_CONSTANTS',
    function($state, telestrationInterface, $window, emitter, EVENT_MAP, Glyphs, PlayManager, TELESTRATIONS_CONSTANTS) {
        ...
        function postlink(scope, elem, attr, API) {
            ...
            telestrationInterface.telestrationSVG = SVG('svg-context-area').size(svgContainer[0].getBoundingClientRect().width, svgContainer[0].getBoundingClientRect().height);
            telestrationInterface.telestrationContainerElement = svgContainer;
            ...
            var telestrationStart = function telestrationStart(mouseEvent) {
                ...
                telestrationInterface.telestrationSVG.size(svgContainer[0].getBoundingClientRect().width, svgContainer[0].getBoundingClientRect().height);
            ...
            var timeUpdateHandler = function timeUpdateHandler(e) {
                ...
                scope.telestrationsEntity.forEach(function(savedTelestration) {
                    // if (savedTelestration.glyphs.length) console.log('telestration time', savedTelestration.time, 'event time', e.detail.time);
                    // TODO: Check if the telestration is within 250 ms of the current time
                    var timeDelta = Math.abs(e.detail.time - savedTelestration.time);
                    // if (savedTelestration.glyphs.length) console.log('timedelta', timeDelta, TELESTRATIONS_CONSTANTS.MAX_TIME_DELTA, savedTelestration.shown);
                    if ((timeDelta <= TELESTRATIONS_CONSTANTS.MAX_TIME_DELTA) && !savedTelestration.shown && savedTelestration.glyphs.length) {
                        // console.log('display telestration');
                        isPaused = true;
                        // make sure svg is the correct size when drawing each time
                        telestrationInterface.telestrationSVG.size(svgContainer[0].getBoundingClientRect().width, svgContainer[0].getBoundingClientRect().height);
            ...
            var removeEventHandlers = function removeEventHandlers() {
                telestrationInterface.telestrationSVG.mousedown(null);
            ...
            telestrationInterface.telestrationSVG.mousedown(telestrationStart);
