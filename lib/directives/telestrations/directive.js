...
module.exports = [
    '$state', '$window', '$timeout', 'EventEmitter', 'EVENT_MAP', 'GlyphFactory', 'TELESTRATIONS_CONSTANTS', 'VideoPlayerEventEmitter', 'TELESTRATION_PERMISSIONS',
    function($state, $window, $timeout, emitter, EVENT_MAP, Glyphs, TELESTRATIONS_CONSTANTS, videoPlayerEventEmitter, TELESTRATION_PERMISSIONS) {
        ...
        function postlink(scope, element, attributes, telestrationsController) {
            ...
            var telestrationStart = function telestrationStart(mouseEvent) {
                ...
                if (!telestrationsController.isActive) return;
                ...
                // create new glyph
                var newGlyph = telestration.addGlyph(telestrationsController.selectedGlyphType, telestrationsController.telestrationSVGContainer, telestrationsController.telestrationSVG, telestrationsController.selectedGlyphColor);
            ...
            var addMouseHandlers = function addMouseHandlers() {
                // add click handler
                removeMouseHandlers();
                console.log('addMouseHandlers');
                telestrationsController.telestrationSVGContainer.on('mousedown', telestrationStart);
                telestrationsController.telestrationSVGContainer.on('mouseup', validateCurrentTelestration);
            };

            var removeMouseHandlers = function removeMouseHandlers() {
                console.log('removeMouseHandlers');
                telestrationsController.telestrationSVGContainer.off('mousedown', telestrationStart);
                telestrationsController.telestrationSVGContainer.off('mouseup', validateCurrentTelestration);
            };
            ...
            if (telestrationsController.permissions === TELESTRATION_PERMISSIONS.EDIT) {
                ...
                telestrationsController.telestrationSVGContainer.on('mouseenter', addMouseHandlers);
                telestrationsController.telestrationSVGContainer.on('mouseleave', removeMouseHandlers);

                telestrationsController.telestrationSVGContainer.on('telestration:onEditModeBlur', function onClickAddMouseHandlers(event) {
                    self.console.log('containerElement received telestration:onEditModeBlur event');
                    telestrationsController.telestrationSVGContainer.off('click', addMouseHandlers);
                    telestrationsController.telestrationSVGContainer.on('click', addMouseHandlers);
                });
            }
