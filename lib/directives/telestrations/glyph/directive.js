    ...
    function($window, videoPlayerEventEmitter, TELESTRATION_PERMISSIONS) {
        return {
            ...
            link: function($scope, elem, attr, ctrls) {
                 ...
                 */
                var getConstraintFn = function getConstraintFn(glyphDimensionsFn) {

                    return function isVertexBounded(x, y) {

                        var containerBox = telestrationsCtrl.telestrationSVGContainer[0].getBoundingClientRect();
                        if (glyphDimensionsFn) return (x > 0) && (x < containerBox.width - glyphDimensionsFn().width) && (y > 0) && (y < containerBox.height - glyphDimensions().height);
                ...
                if (telestrationsCtrl.permissions === TELESTRATION_PERMISSIONS.EDIT) {
                    $scope.glyphObject.setDraggableConstraintFn(getConstraintFn($scope.glyphObject.getDimensions));
