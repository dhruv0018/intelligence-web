...
Instructions.controller('Coach.Game.Instructions.controller', [
    ...
    function controller($scope,
        ...
        $scope.$watch('remainingBreakdowns', () => {
            $scope.noBreakdownsRemaining = $scope.remainingBreakdowns.planGamesRemaining <= 0 && $scope.remainingBreakdowns.packageGamesRemaining <= 0;
            if ($scope.remainingBreakdowns.plan && $scope.remainingBreakdowns.plan.maxAnyGames) {
                $scope.hasMaxWeeklyGames = true;
            }
        });
        ...
        function updateRemainingBreakdowns() {
            teams.load(teamIdForThisGame).then(teamData => {
                $scope.activePlan = teams.get(teamIdForThisGame).getActivePlan() || {};
                $scope.activePackage = teams.get(teamIdForThisGame).getActivePackage() || {};
                teams.getRemainingBreakdowns(teamIdForThisGame).then(breakdownData => {
                    session.currentUser.remainingBreakdowns = breakdownData;
                    $scope.remainingBreakdowns = breakdownData;
                });
            });
        }
        ...
        $scope.openBreakdownPurchaseModal = function() {
            breakdownPurchaseModal.open($scope.products, updateRemainingBreakdowns);
        };
