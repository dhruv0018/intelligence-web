    ...
    function directive(ROLES, teams, users, games, session, ShareFilmModal, GAME_TYPES_IDS, GAME_TYPES) {
        ...
        var film = {
            ...
            link: function($scope, element, attrs) {
                ...
                $scope.isShared = $scope.film.isSharedWithUser(currentUser) || // Game
                                  (!$scope.isGame && !$scope.isUploader && $scope.film.isSharedWithTeam()); // Reel

                $scope.isCoachOnUploaderTeam = currentUser.is(ROLES.COACH) &&
                                               currentUser.currentRole.teamId === film.uploaderTeamId;

                console.log('isUploader: ', $scope.isUploader, 'isShared', $scope.isShared, 'isCoachOnUploaderTeam', $scope.isCoachOnUploaderTeam);
                $scope.uploaderName = '';
