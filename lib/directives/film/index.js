...
Film.directive('krossoverFilm', [
    'ROLES', 'TeamsFactory', 'UsersFactory', 'GamesFactory', 'ReelsFactory', 'SessionService', 'ShareFilm.Modal', 'GAME_TYPES_IDS', 'GAME_TYPES',
    function directive(ROLES, teams, users, games, reels, session, ShareFilmModal, GAME_TYPES_IDS, GAME_TYPES) {
        ...
        var film = {
            ...
            link: function($scope, element, attrs) {
                ...
                $scope.isShared = $scope.film.isSharedWithUser(currentUser) ||
                                  (!$scope.isGame && !$scope.isUploader && $scope.film.isSharedWithTeam());

                $scope.isCoachOnUploaderTeam = currentUser.is(ROLES.COACH) &&
                ...
                if ($scope.isShared) {
                    ...
                    } else if ($scope.film.isSharedWithTeam()) {
                        uploaderUser = users.get($scope.film.getTeamShare().userId);
                    }
                } else {
