...
function CustomTagsDropdownController (
    ...
    // Determine which type of plays to use
    let playsFactory;

    if($scope.forSelfEditor) {
        playsFactory = selfEditedPlays;
    } else {
        playsFactory = plays;
    }
    ...
    $scope.focusSearchBar = function(isOpen) {
        if(!isOpen) {
            window.setTimeout(() => {
                document.getElementById('custom-tags-search-cta').focus();
            }, 1);
        }
    };
    ...
    $scope.updateFilteredTags = function() {
        $scope.filteredTags = $filter('filter')($scope.availableTags, $scope.filters, false);
    ...
    $scope.tagAlreadyExists = function(name) {
        ...
        return $scope.customTags.find( tag => {
            return (tag.name).toUpperCase() === name.toUpperCase();
        });
    };
    ...
    $scope.applyTags = function() {
        ...
        $rootScope.$broadcast('self-edited-tags-applied', $scope.selectedTags);
        customtags.batchSave($scope.selectedTags).then( tags => {
            ...
            playsFactory.batchSave($scope.plays).then(function deselectPlays() {
        ...
        let selectedTagNames = $scope.selectedTags.map(tag => tag.name);
        let selectedTagCount = $scope.selectedTags.length + ' tag(s) applied';

        analytics.track('Tag Applied', {
            'Number of Tags Added': selectedTagCount,
            'Tag Name': selectedTagNames
        });
    };
