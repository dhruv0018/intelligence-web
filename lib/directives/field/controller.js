...
class FieldController {
    ...
    onSelect(value) {
        ...
        let inputArr = Array.prototype.slice.call(document.querySelectorAll('input.mousetrap'));
        let curInput = document.querySelector(`#field-${this.scope.field.index}`);
        if(inputArr.length >0 && curInput){
            let curIndx = inputArr.indexOf(curInput);
            if(curIndx+1 < inputArr.length){
                window.setTimeout(()=>{
                    inputArr[curIndx+1].focus();
                }, 250);
            }
        }
    }
