...
Play.controller('Play.controller', [
    '$scope', '$modal', 'ROLES', 'SessionService', 'TeamsFactory', 'PlayManager', 'IndexingService', 'EventManager', 'VideoPlayerInstance', 'VG_EVENTS', 'VG_STATES',
    function controller($scope, $modal, ROLES, session, teams, playManager, indexing, eventManager, videoPlayerInstance, VG_EVENTS, VG_STATES) {
        ...
        $scope.selectPlay = function() {
            ...
            if (playManager.playAllPlays) {
                var waitTime = 200;
                var waitForCurrentElementToChangeInDom = setInterval(function() {

                    if ($scope.playElement[0].getElementsByClassName('playHide').length) {
                        waitTime *= 1.2;
                        if (waitTime > 600) clearInterval(waitForCurrentElementToChangeInDom);
                        return;
                    }

                    clearInterval(waitForCurrentElementToChangeInDom);
