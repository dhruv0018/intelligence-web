...
Play.controller('Play.controller', [
    '$scope', '$modal', 'ROLES', 'SessionService', 'TeamsFactory', 'PlayManager', 'EventManager', 'VideoPlayerInstance', 'VG_EVENTS',
    function controller($scope, $modal, ROLES, session, teams, playManager, eventManager, videoplayer, VG_EVENTS) {
        ...
        $scope.playPlay = function() {
            ...
            videoplayer.then(function(vp) {
                ...
                if ($scope.playPlayed || vp.isPlayerReady()) {
                    vp.play();
                    //hack to prevent accordian from opening/keep open
                    $scope.isOpen = false;
                } else {
