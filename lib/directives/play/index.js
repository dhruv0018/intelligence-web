...
Play.directive('krossoverPlay', [
    ...
    function directive(session, ROLES, leagues, games, teams, emitter, EVENT_MAP, playManager, mediator) {
        ...
        function link($scope, element, attributes) {
            ...
            $scope.highlightedEventId = null;

            var timeUpdateHandler = function(e) {
                if (playManager.current.id === $scope.play.id) {
                    var currentVideoTime = e.detail.time;
                    var selectedEvent = null;
                    for (var index = 0; index < $scope.play.events.length; index++) {
                        var event = $scope.play.events[index];
                        var eventOccurTime = event.time - $scope.play.startTime;
                        if (eventOccurTime < currentVideoTime) {
                            selectedEvent = event;
                        } else {
                            $scope.highlightedEventId = selectedEvent.id;
                            break;
                        }
                    }
                }
