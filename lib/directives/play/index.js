...
Play.controller('Play.controller', [
    '$scope', '$state', '$stateParams', '$modal', '$interval', 'ROLES', 'SessionService', 'config', 'TeamsFactory', 'PlayManager', 'IndexingService', 'EventManager', 'VideoPlayerInstance', 'VG_EVENTS', 'VG_STATES', 'VideoElement', 'VIEWPORTS',
    function controller($scope, $state, $stateParams, $modal, $interval, ROLES, session, config, teams, playManager, indexing, eventManager, videoPlayerInstance, VG_EVENTS, VG_STATES, videoPlayerElement, VIEWPORTS) {
        ...
        $scope.selectMobileClipPlay = function() {
            ...
            if ($state.current.name === 'Clips') {
                ...
                $state.go('Clips', {id: $scope.play.id, reel: $stateParams.id});
            }
        };
