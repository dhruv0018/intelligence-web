...
Play.directive('krossoverPlay', [
    'SessionService', 'ROLES', 'LeaguesFactory', 'GamesFactory', 'TeamsFactory', 'EventEmitter', 'EVENT_MAP', 'EventManager', 'PlayManager', 'PlayVideoMediator',
    function directive(session, ROLES, leagues, games, teams, emitter, EVENT_MAP, eventManager, playManager, mediator) {
        ...
        function link($scope, element, attributes) {
            ...
            var currentEventIndex = $scope.play.events.length - 1;

            var timeUpdateHandler = function(e) {

                var currentVideoTime = e.detail.time;

                /* Loop through the plays events backwards. */
                for (var eventIndex = $scope.play.events.length - 1; eventIndex >= 0 ; eventIndex--) {

                    var event = $scope.play.events[eventIndex];
                    var eventTime = event.time;

                    /* If the play has been clipped, adjust the event time. */
                    if ($scope.play.clip) eventTime -= $scope.play.startTime;

                    /* Has the video time passed the event time? */
                    if (currentVideoTime > eventTime) {
            ...
            $scope.$watch('playManager.current', watchCurrentPlay);

            function watchCurrentPlay(currentPlay) {

                if ($scope.play === currentPlay) {

                    currentEventIndex = $scope.play.events.indexOf(eventManager.current);
