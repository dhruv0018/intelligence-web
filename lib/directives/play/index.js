...
Play.directive('krossoverPlay', [
    'TeamsFactory', 'SessionService', 'ROLES', 'LeaguesFactory', 'GamesFactory', 'TeamsFactory',
    function directive(teams, session, ROLES, leaguesFactory, gamesFactory, teamsFactory) {
        ...
        function link($scope, element, attributes) {
            ...
            var homeTeamId = gamesFactory.get(gameId).teamId;
            var opposingTeamId = gamesFactory.get(gameId).opposingTeamId;

            $scope.team = getTeam(homeTeamId);
            $scope.opposingTeam = getTeam(opposingTeamId);

            $scope.league = leaguesFactory.get($scope.team.leagueId);
            $scope.game = gamesFactory.get(play.gameId);
            ...
            $scope.isTeamMember = $scope.team.isMember(session.currentUser.id);
...
Play.controller('Play.controller', [
    '$scope', '$modal', '$interval', 'ROLES', 'SessionService', 'config', 'TeamsFactory', 'PlayManager', 'IndexingService', 'EventManager', 'VideoPlayerInstance', 'VG_EVENTS', 'VG_STATES', 'VideoElement', 'VIEWPORTS',
    function controller($scope, $modal, $interval, ROLES, session, config, teams, playManager, indexing, eventManager, videoPlayerInstance, VG_EVENTS, VG_STATES, videoPlayerElement, VIEWPORTS) {
