...
function WSCLinkController (
    ...
    $scope.createWSCHighlight = function() {
        ...
        // Add sportId param, defaults to basketball
        let sportId = SPORTS.BASKETBALL.id;
        if ($scope.isAthlete && $scope.seasonId) {
            sportId = $scope.selectedTeam.sportId;
        } else {
            sportId = teams.get(session.getCurrentTeamId()).sportId;
        }
        wscURL = `${wscURL}&sportId=${sportId}`;
