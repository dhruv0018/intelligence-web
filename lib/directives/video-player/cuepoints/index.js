...
CuePoints.directive('cuePoints', [
    ...
    function(CUEPOINT_CONSTANTS, videoPlayerEventEmitter, VIDEO_PLAYER_EVENTS, cuePointEventEmitter) {
        ...
        function cuePointLink($scope, $element, $attributes, $controllers) {
            ...
            function emitCuePointHitOnSeeking(cuePoint) {

                emitCuePointHitOnNextCycle(cuePoint);
            }
            let boundEmitCuePointHitOnSeeking;
                ...
                for (let index in $scope.points) {
                    ...
                    if (timeDelta <= CUEPOINT_CONSTANTS.MAX_TIME_DELTA && !cuePoint.hit && !cuePointTimeRangeLock) {
                        ...
                        if (cuePoint.type === CUEPOINT_CONSTANTS.TYPES.TELESTRATION) {
                            ...
                            boundEmitCuePointHitOnSeeking = emitCuePointHitOnSeeking.bind(this, cuePoint);

                            // pause, seek, then emit cuePoint event.
                            videogularAPI.pause();

                            videoPlayerEventEmitter.on(VIDEO_PLAYER_EVENTS.ON_SEEKING, boundEmitCuePointHitOnSeeking);
            ...
            function emitCuePointHitOnNextCycle(cuePoint) {
                ...
                }, 0); // use a larger timeout for slow browsers (i.e. safari) TODO: Reduce to 0 or use promise on videoplayer controls

                videoPlayerEventEmitter.removeListener(VIDEO_PLAYER_EVENTS.ON_SEEKING, boundEmitCuePointHitOnSeeking);
            }
