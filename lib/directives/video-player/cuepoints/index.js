...
function CuePointsDirective(
    ...
    function cuePointLink($scope, $element, $attributes, $controllers) {
        ...
        const chromeFixTimeOffset = 0.001; // 1ms;
        function videoCanPlayCurrentTime(cuePoint) {

            videoElement.removeEventListener('canplay', boundVideoCanPlayCurrentTime);

            runFunctionOnNextCycle(chromeFrameFix.bind(this, cuePoint));
        }

        function chromeFrameFix(cuePoint) {

            /* NOTE: This is a hack to update the video's frame (specifically in chrome) once
            * the video's currentTime has been updated and the video `canplay` */

            videoElement.currentTime = videoElement.currentTime + chromeFixTimeOffset;
