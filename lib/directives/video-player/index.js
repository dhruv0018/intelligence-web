...
VideoPlayer.constant('VIDEO_PLAYER_CONFIG', {
        ...
        $document.ready(() => {
            ...
            function fitVideo() {
                ...
                if (vgFullscreen.isFullScreen()) {
                    ...
                    videoPlayer.videogularElement.css('width', '100%');
                    videoPlayer.videogularElement.css('height', '100%');
                    videoPlayer.mediaElement.css('width', '100%');
                    videoPlayer.mediaElement.css('height', 'calc(100% - ' + VIDEO_PLAYER_CONFIG.CONTROLS_HEIGHT + 'px)');
                    videoPlayer.mediaElement.css('max-width', '100%');
                    videoPlayer.mediaElement.css('max-height', 'calc(100% - ' + VIDEO_PLAYER_CONFIG.CONTROLS_HEIGHT + 'px)');

                } else {

                    // Aspect ratio is calculated before adding the height required for the controls
                    let videoRect = videoPlayer.videogularElement.parent()[0].getBoundingClientRect();
                    let currentVideoWidth = videoRect.width;
                    let currentVideoHeight = videoRect.height;

                    let parentWithHeight = getParentWithHeight(videoPlayer.videogularElement);
                    ...
                    * only fit the player within the parent's height if on desktop */
                    if ($rootScope.viewport.name === VIEWPORTS.DESKTOP && properPlayerHeight > parentHeight) {
                    ...
                    videoPlayer.videogularElement.css('width', properWidth + 'px');
                    videoPlayer.videogularElement.css('height', properPlayerHeight + 'px');
                    videoPlayer.mediaElement.css('width', properWidth + 'px');
                    videoPlayer.mediaElement.css('height', properVideoHeight + 'px');
                    videoPlayer.mediaElement.css('max-width', properWidth + 'px');
                    videoPlayer.mediaElement.css('max-height', properVideoHeight + 'px');
                }
            }
