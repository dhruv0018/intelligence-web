...
function VideoPlayerDirective (
    ...
    function link($scope, element, attributes, videoPlayer) {
        ...
        $document.ready(() => {
            ...
            videoPlayerEventEmitter.on(VIDEO_PLAYER_EVENTS.FULLSCREEN, function(event){

                let overlayElements = videoPlayer.overlayElements;

                overlayElements.forEach(overlayElement => {

                    //Depending on full screen or not, find the element to prepend to
                    let parentElement = (event.isFullScreen) ? document.querySelector('videogular') :
                                                                document.getElementsByClassName('indexing')[0].
                                                                    getElementsByClassName('content-full')[0];

                    //Prepend indexing block into parent element
                    parentElement.insertBefore( overlayElement, parentElement.firstChild );

                    //Toggle fullscreen class
                    overlayElement.classList.toggle('fullscreen', event.isFullScreen);
