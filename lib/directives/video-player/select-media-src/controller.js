...
function SelectMediaSrcController (
    ...
    $scope.onVideoQualitySelect = (selectedTranscodeProfile) => {
        ...
        if (selectedTranscodeProfile === $scope.currentTranscodeProfile) return;

        $scope.currentTranscodeProfile = selectedTranscodeProfile;
        selectedBitrate = $scope.currentTranscodeProfile.targetBitrate;

        video = $scope.videoPlayer.video;
        let currentTranscodeProfileId = $scope.currentTranscodeProfile.id;
