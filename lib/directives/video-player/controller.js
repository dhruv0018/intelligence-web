...
function VideoPlayerController (
    ...
    $scope.userIsAthlete = currentUser.is(ROLES.ATHLETE);
    $scope.userIsCoach = currentUser.is(ROLES.COACH);
    ...
    let onChangeSource = (source) => {
        ...
        this.canPlay = false;
        videoPlayerEventEmitter.emit(VIDEO_PLAYER_EVENTS.ON_SOURCE_CHANGE, source);
    };
    ...
    let onFullscreenChange = () => {
        ...
        $scope.isFullscreen = document.fullscreenEnabled && this.fullscreenElement === videogularElement;
        videoPlayerEventEmitter.emit(VIDEO_PLAYER_EVENTS.FULLSCREEN, {isFullScreen: $scope.isFullscreen});
    };
    ...
    videoPlayer.setVideoFragment = (video, start, end) => {
        ...
        if (!video) {
            ...
            mediaElement.addEventListener('pause', function onFragmentPause(event) {
                if (videoPlayer.currentTime >= end) {
                    videoPlayerEventEmitter.emit(VIDEO_PLAYER_EVENTS.ON_FRAGMENT_END, event);
                    mediaElement.removeEventListener('pause', onFragmentPause);
                }
                videoPlayerEventEmitter.on(VIDEO_PLAYER_EVENTS.ON_SOURCE_CHANGE, function onSourceChange(source) {
                    mediaElement.removeEventListener('pause', onFragmentPause);
                });
            });
        }
