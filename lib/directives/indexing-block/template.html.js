...
<div class="indexing-block" ng-class="{ 'fullscreen': fullScreenEnabled }">
    ...
    <form class="indexing-form" name="formIndexing" role="form" novalidate>
        ...
        <!-- Tags -->
            ...
            <button class="tag" data-ng-repeat="tag in tags.current | restrictToPeriodTags track by $index" data-keybinding="{{ tag.keyboardShortcut.toLowerCase() }}" data-ng-click="indexing.selectTag(tag.id, game)" tabindex="{{ $index+1 }}">{{ tag.name }} ({{ tag.keyboardShortcut.toLowerCase() }})</button>
