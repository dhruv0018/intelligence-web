    ...
    <form class="indexing-form" name="formIndexing" role="form" novalidate>
        ...
        <div class="script">
            ...
            <span ng-repeat="field in event.current.indexerFields">
                ...
                <team-player-dropdown-field ng-if="field.type === 'PLAYER_TEAM_DROPDOWN' " event="event.current" field="field"></team-player-dropdown-field>
                <player-dropdown-field ng-if="field.type === 'PLAYER_DROPDOWN' " event="event.current" field="field"></player-dropdown-field>
                <team-dropdown-field ng-if="field.type === 'TEAM_DROPDOWN'" event="event.current" field="field"></team-dropdown-field>
                <gap-dropdown-field ng-if="field.type === 'GAP'" event="event.current" field="field"></gap-dropdown-field>
                <formation-dropdown-field ng-if="field.type === 'FORMATION'" event="event.current" field="field"></formation-dropdown-field>
                <passing-zone-dropdown-field ng-if="field.type === 'PASSING_ZONE'" event="event.current" field="field"></passing-zone-dropdown-field>
                <!--todo: tech debt incurred: subbing out the only text field for a yard field because it IS a yard field-->
                <yard-dropdown-field ng-if="field.type === 'YARD' || field.type === 'TEXT' " event="event.current" field="field"></yard-dropdown-field>
                <arena-dropdown-field ng-if="field.type === 'ARENA'" event="event.current" field="field"></arena-dropdown-field>
                <dropdown-field ng-if="field.type === 'DROPDOWN'" event="event.current" field="field"></dropdown-field>
            </span>
        </div>
