...
<div class="indexing-block" ng-class="{ 'fullscreen': fullScreenEnabled }">
    ...
    <form class="indexing-form" name="formIndexing" role="form" novalidate>
        ...
        <!-- Tags -->
        <div class="tags group-tags" data-ng-if="indexing.isIndexing && indexing.showTags">

            <button class="tag group-tags-btn" ng-if="tag.isGroup" data-ng-repeat="tag in tags.current" data-keybinding="{{ tag.keyboardShortcut.toLowerCase() }}" data-ng-click="indexing.selectTag(tag.id, game)" tabindex="{{ $index+1 }}">{{ tag.name }} ({{ tag.keyboardShortcut.toLowerCase() }})</button>

        </div>

        <div class="tags" data-ng-if="indexing.isIndexing && indexing.showTags">

            <button class="tag" ng-if="!tag.isGroup" data-ng-repeat="tag in tags.current" data-keybinding="{{ tag.keyboardShortcut.toLowerCase() }}" data-ng-click="indexing.selectTag(tag.id, game)" tabindex="{{ $index+1 }}">{{ tag.name }} ({{ tag.keyboardShortcut.toLowerCase() }})</button>
        ...
        <div class="controls">
            ...
            <button
                class="btn-next"
                ng-show="indexing.isIndexing && indexing.showScript && !event.current.isEnd"
                ng-click="indexing.returnFromNext()"
