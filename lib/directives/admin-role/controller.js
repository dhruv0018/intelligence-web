...
function AdminRoleController (
    ...
    $scope.getFilmExchangePrivileges = function(){
        if($scope.role.id){
            users.getFilmExchangePrivileges(currentUser.id).then(
                function(data){
    ...
    $scope.addNewFilmExchangePrivilege = function(){
        users.addFilmExchangePrivilege(currentUser.id, $scope.newPrivilege).then(
            function() {
                alerts.add({
                    type: 'success',
    ...
    $scope.deleteFilmExchangePrivilege = function(privilege){
        ...
        removePrivilegeModal.result.then(function removePrivilegeModalCallback() {
            users.deleteFilmExchangePrivilege(currentUser.id, privilege.id).then(
                function() {
                    alerts.add({
                        type: 'success',
                        message: 'Changes Saved!'
