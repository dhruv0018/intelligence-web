angular.module('GlyphShapes.ShadowShape', []).factory('ShadowShapeFactory', [
    ...
    function($location, telestrationInterface, Shape) {
        ...
        function addMaskLayers() {
            // Semi-opaque layer
            telestrationSVGMaskBlack = telestrationInterface.telestrationSVG.rect('100%', '100%').attr({ fill: '#000' }).opacity(0.4);

            // Mask layer
            telestrationSVGMask = telestrationInterface.telestrationSVG.mask();
            SVGMaskWhite = telestrationInterface.telestrationSVG.rect('100%', '100%').attr({ fill: '#fff' });
