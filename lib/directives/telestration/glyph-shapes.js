...
GlyphShapes.factory('GlyphShapeRenderer', [
    ...
    function(telestrationInterface, glyphShapesConstants, TELESTRATION_TYPES, $location) {
        ...
        var BORDER_WIDTH = glyphShapesConstants.BORDER_WIDTH;
        var T_BAR_LENGTH = glyphShapesConstants.T_BAR_LENGTH;
        var ARROW_SIDE_LENGTH = glyphShapesConstants.ARROW_SIDE_LENGTH;
        var ARROW_HEIGHT = Math.sqrt(ARROW_SIDE_LENGTH * ARROW_SIDE_LENGTH + (ARROW_SIDE_LENGTH / 2) * (ARROW_SIDE_LENGTH / 2));

        function renderArrow(vertices, color, text) {
        ...
        function renderTBar(vertices, color, text) {
        ...
        function renderCircle(vertices, color, text) {
        ...
        function renderShadowCircle(vertices, color, text) {
            ...
            if (typeof this.currentShape === 'undefined' && telestrationInterface.telestrationSVG) {
                this.currentShape = 1;//telestrationInterface.telestrationSVG.circle().opacity(0);
        ...
        function renderCone(vertices, color, text) {
        ...
        function renderFreehand(vertices, color, text) {
        ...
        function renderTextBox(vertices, color, text) {
        ...
        function ShapeRenderer(glyphElement, type) {
            ...
            switch (self.type) {
                ...
                case TELESTRATION_TYPES.ARROW:
                    self.renderShape = renderArrow.bind(self);
                    break;

                case TELESTRATION_TYPES.T_BAR:
                    self.renderShape = renderTBar.bind(self);
                    break;

                case TELESTRATION_TYPES.CONE:
                    self.renderShape = renderCone.bind(self);
                    break;

                case TELESTRATION_TYPES.FREEHAND:
                    self.renderShape = renderFreehand.bind(self);
                    break;

                case TELESTRATION_TYPES.CIRCLE:
                    self.renderShape = renderCircle.bind(self);
                    break;

                case TELESTRATION_TYPES.SHADOW_CIRCLE:
                    self.renderShape = renderShadowCircle.bind(self);
                    break;

                case TELESTRATION_TYPES.TEXT_TOOL:
                    self.renderShape = renderTextBox.bind(self);
                    break;
            }
        }
