...
module.exports = [
    '$state', 'TelestrationInterface', '$window', 'EventEmitter', 'EVENT_MAP', 'GlyphFactory', 'TelestrationEntity',
    function($state, telestrationInterface, $window, emitter, EVENT_MAP, Glyphs, TelestrationEntity) {
        ...
        function postlink(scope, elem, attr, API) {
            ...
            var telestrationStart = function telestrationStart(mouseEvent) {
                ...
                // TODO: This is temporary scaffolding for rest of model creation
                var currentVideoPlayerTime = 0; // TODO: Get currentVideoPlayerTime from videoPlayer
                var telestration = telestrationEntityModel.getTelestration(currentVideoPlayerTime, scope.film.id);
                    ...
                    if (telestration.time <= e.detail.time && !telestration.shown) {
                        ...
                        if (telestration.glyphs) {
                            telestration.glyphs.forEach(function(tempGlyph) {
                                var glyph = telestration.addGlyph(telestrationInterface.selectedGlyphType, telestrationInterface.telestrationSVG, telestrationInterface.selectedGlyphColor);
