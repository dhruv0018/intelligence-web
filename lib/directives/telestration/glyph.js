...
Glyph.factory('GlyphShapeRenderer', [
    ...
    function(telestrationInterface) {
        ...
        function ShapeRenderer(glyphElement) {
            ...
            self.registerListeners = function() {
                self.currentShape.on('click', function(mouseEvent) {
                    if (telestrationInterface.telestrationSVG && self.glyphElement) {
                        telestrationInterface.telestrationSVG.style({ 'z-index': 1 });
                        self.glyphElement[0].style.zIndex = 2;
                        self.glyphElement[0].style.backgroundColor = '#ffffff';
                        self.glyphElement[0].style.opactiy = '0.5';
                    }
                ...
                //    if (typeof self.onMovedHandler === 'function') self.onMovedHandler(mouseEvent);
            };
        }
        ...
        ShapeRenderer.prototype.renderCircle = function renderCircle(startPoint, endPoint, radius, color) {
            if (typeof this.currentShape === 'undefined' && telestrationInterface.telestrationSVG) {
        ...
        ShapeRenderer.prototype.renderShadowCircle = function renderShadowCircle(startPoint, endPoint, radius, color) {
            if (typeof this.currentShape === 'undefined' && telestrationInterface.telestrationSVG) {
                this.currentShape = telestrationInterface.telestrationSVG.circle();
                this.registerListeners();
            }
        ...
        ShapeRenderer.prototype.renderCone = function renderCone(startPoint, endPoint, coneOpeningWidth, color) {
        ...
        ShapeRenderer.prototype.renderTextBox = function renderTextBox(startPoint, endPoint, text, color) {
            ...
            var updatedSVGShape = angular.element('<div style="position:absolute;display:inline-block;overflow:hidden;background-color:#ffffcc;padding:8px;line-height:14px;letter-spacing:0px;font-size:16px;height:' + offsetY + 'px;width:' + offsetX + 'px;border:solid ' + 8 + 'px ' + (color || this.defaultColor) + ';">' +
                '<span>' + text + '</span>' +
            '</div>');
            this.glyphElement.append(updatedSVGShape);
    ...
    function(TELESTRATION_TYPES, telestrationInterface, GlyphShapeRenderer, $location) {
        return {
            ...
            link: function(scope, elem, attr) {
                ...
                //Event handling for moving the glyph. Update the verticies and redraw
                ...
                elem.bind('mousemouse', function(e) {
                    if (glyphSelected) {
                        e.stopPropagation();
                    }
                });
                    ...
                    if (startPoint && startPoint.length && endPoint && endPoint.length) {
                        ...
                        */
                        ...
                        switch (scope.glyphObject.getType()) {
                            ...
                            case TELESTRATION_TYPES.CIRCLE:
                                thisGlyph.renderCircle(startPoint, endPoint, pointDistance / 2);
                                break;

                            case TELESTRATION_TYPES.SHADOW_CIRCLE:
                                thisGlyph.renderShadowCircle(startPoint, endPoint, pointDistance / 2);
                                break;

                            case TELESTRATION_TYPES.CONE:
                                thisGlyph.renderCone(startPoint, endPoint, pointDistance / 2);
                                break;
