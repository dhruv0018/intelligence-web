    ...
    function($window, GlyphShapeRenderer, telestrationInterface) {
        return {
            ...
            link: function($scope, elem, attr, glyphEditor) {
                ...
                var rerenderGlyph = function rerenderGlyph() {
                    drawGlyph();
                };

                // Rerender the glyph on window resize
                angular.element($window).bind('resize', rerenderGlyph);
                ...
                elem.on('$destroy', function() {
                    thisGlyph.destroy();
                    $window.removeEventListener('resize', rerenderGlyph);
                    glyphEditor.$hide();
                });
            }
        };
