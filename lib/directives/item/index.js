...
Item.controller('ItemController', [
    ...
    function ItemController($scope,
        ...
        $scope.reset = function($event) {
            ...
            //todo not the best
            if ($scope.item.type === 'ARENA') $scope.openArenaModal($event);
        };
        ...
        switch($scope.item.type) {
            ...
            case 'ARENA':
                ...
                /* Scope for template */
                $scope.openArenaModal = openArenaModal;

                $scope.$watch('event.activeEventVariableIndex', onActiveEventVariableIndexChange);

                break;
        }

        // Reference for open arena dialog
        let arenaDialogPromise = null;
        ...
        function onActiveEventVariableIndexChange() {

            // Use case: indexer presses enter or esc to navigate items, modal should close
            if ($scope.item.type === 'ARENA' && $scope.autoAdvance === true) {
        ...
        function openArenaModal(event) {
            ...
            arenaDialogPromise = ArenaDialog.show(event, $scope.item, $scope.league);
        ...
        $scope.determineResetButtonView = function(value) {
                switch($scope.item.type) {
                    ...
                    case 'TEAM_DROPDOWN':
                        ...
                        break;
                    case 'ARENA':
                        $scope.resetButton.text = (value && value.region.id) ? ARENA_REGIONS_BY_ID[value.region.id].description : 'Optional';
                        break;
                    case 'PLAYER_TEAM_DROPDOWN':
