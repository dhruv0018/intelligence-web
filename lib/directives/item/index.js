...
Item.directive('krossoverItem', [
    ...
    function directive(ROLES, $location, $anchorScroll, session, games, tagsets) {
        ...
        function koiItemController($scope) {
            //TODO tags should be their own classes
            $scope.tagId = $scope.event.tagId;
            $scope.tagsetId = $scope.league.tagSetId;
            $scope.tagset = tagsets.get($scope.tagsetId);
            $scope.tags = $scope.tagset.tags;
            $scope.tag = $scope.tags[$scope.tagId];
