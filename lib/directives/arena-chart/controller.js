...
function ArenaChartController(
    ...
    //update the tag info for each game play
    $scope.$on('close-play-breakdown', (event)=>{
        $timeout(()=>{
            game.retrieveArenaEvents().then(data=>{
                this.arenaEvents = data;
            });
        }, 0);
    });
