...
function ArenaChartController(
    ...
    const playsList = plays.getList({gameId});
    ...
    this.onArenaEventClick = (arenaEvent) => {

        let playIds = [arenaEvent.playId];
        this.openBreakdownModal(playIds);
    };

    this.openBreakdownModal = (playIds) => {

        let currentPlays = playIds.map(getPlays);

        function getPlays(playId) {
            return playsList.find((play) => play.id === playId);
        }

        let modalScope = $scope.$new(true);
        modalScope.currentPlays = currentPlays;
        modalScope.game = game;
        modalScope.league = league;

        filmBreakdownModal.open({
            scope: modalScope,
        });
    };
