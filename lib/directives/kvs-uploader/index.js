...
kvsUploader.directive('kvsUploader', [
    '$log', '$window', '$http', '$state', '$rootScope', 'AlertsService', 'ALLOWED_FILE_EXTENSIONS', 'config', 'flowFactory', 'uploadManager',
    function directive($log, $window, $http, $state, $rootScope, alerts, ALLOWED_FILE_EXTENSIONS, config, flow, uploadManager) {
        ...
        function link(scope, element, attributes, controller) {
            ...
            scope.flow.on('complete', function() {
                if (scope.film.description === 'games') {
                    $log.log('--Upload Diagnostic -- Upload Complete for game - ' + scope.film.id + ' at ' + new Date() + ' # of Files: ' + scope.film.flow.files.length);
                }
                            ...
                            if (data && data.guid) {
                                ...
                                if (uploadManager.count() < uploadManager.MAX_UPLOADS) {
                                    scope.film.save().then(function() {
                                        ...
                                        $state.go('Coach.GameArea', {id: scope.film.id});
                                    });
                                } else {
                                    alerts.add({
                                        type: 'danger',
