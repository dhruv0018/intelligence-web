...
function BreakdownController (
    ...
    function changePlay(play) {
        ...
        let result = videoPlayer.setVideo(play.clip);

        const VIDEO_ALREADY_SET = 0;
        const VIDEO_IS_NULL = -1;

        if (result === VIDEO_ALREADY_SET) deferred.resolve();
        else if (result === VIDEO_IS_NULL) deferred.reject();
