...
function BreakdownController (
    ...
    function onEventSelect (event) {
        ...
        if (playManager.current.id === event.playId) {

            let videoTime = PlaylistManager.getRelativeClipTime(playManager.current, event);
            ...
            changePlay(play).then(() => {
                ...
                videoPlayer.seekTime(PlaylistManager.getRelativeClipTime(play, event));
            });
        }
    }
    ...
    function onDestroy() {
        ...
        playlistEventEmitter.removeListener(EVENT.PLAYLIST.PLAY.WATCH, onWatchPlay);
        VideoPlayerEventEmitter.removeListener(VIDEO_PLAYER_EVENTS.ON_CLIP_COMPLETE, onCompleteVideo);
