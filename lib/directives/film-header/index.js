...
filmHeader.directive('filmHeader', [
    '$timeout', 'TeamsFactory', 'ReelsFactory', 'GAME_TYPES_IDS', 'GAME_TYPES', 'SessionService', 'AuthenticationService', 'ROLES', 'ShareFilm.Modal', 'BasicModals', 'AccountService',
    function directive($timeout, teams, reels, GAME_TYPES_IDS, GAME_TYPES, session, auth, ROLES, ShareFilmModal, modals, account) {
        ...
        function link($scope, element, attrs) {
            ...
            $scope.toggleReelPublishing = function() {
                ...
                if ($scope.film.isFeatured($scope.user)) {
                    $scope.user.removeFeaturedReelId();
                    $scope.user.save();
                } else if (!$scope.film.isFeatured($scope.user)) {
            ...
            $scope.savePublishedReel = function() {
                ...
                $scope.user.setFeaturedReelId($scope.film.id);
                $scope.user.save();
            };
