...
Videoplayer.controller('Videoplayer.controller', [
    '$window', '$element', '$rootScope', 'config', '$scope', '$state', '$sce', '$modal', 'vgFullscreen', 'Videoplayer', 'VideoPlayerInstance', 'PlayManager', 'EventManager', 'EventEmitter', 'VG_CONFIG', 'VG_STATES', 'DEVICE',
    function controller($window, $element, $rootScope, config, $scope, $state, $sce, $modal, vgFullscreen, videoplayer, videoPlayerInstance, playManager, eventManager, emitter, VG_CONFIG, VG_STATES, DEVICE) {
        ...
        function fitVideo(videogularElement, mediaElement) {
            ...
            var videoParent = $element.parent();
            var videoRect = videoParent[0].getBoundingClientRect();
            var currentVideoWidth = videoRect.width;
            var currentVideoHeight = videoRect.height;

            var parentWithHeight = getParentWithHeight($element);
            ...
            if (!parentWithHeight) {
                ...
                * only fit the player within the parent's height if on desktop */
                if ($rootScope.viewport.name === 'Desktop' && properPlayerHeight > parentHeight) {
                    ...
                    properVideoHeight = properPlayerHeight;
