...
Videoplayer.factory('VideoPlayerInstance', [
    ...
    function($q, vAPI, VG_EVENTS) {
        ...
        setTimeout(function() {
            ...
            deferred.notify(waitTime, vAPI);
            if (vAPI) {
                if (vAPI.isPlayerReady()) {
                    deferred.resolve(vAPI);
                } else {
                    vAPI.on(VG_EVENTS.ON_PLAYER_READY, function() {
                        deferred.resolve(vAPI);
                    });
                }
            }
...
Videoplayer.value('VideoPlayerAPI', null);
...
Videoplayer.directive('krossoverVideoplayer', [
    ...
    function($sce) {
        ...
        function link($scope, element, attributes) {
            ...
            console.log('v', $scope.videoElement);
        }
...
Videoplayer.controller('Videoplayer.controller', [
    'config', 'package', '$scope', '$state', '$sce', '$modal', 'VideoPlayerInstance', 'PlayManager', 'EventManager', 'VideoPlayerAPI',
    function controller(config, package, $scope, $state, $sce, $modal, videoPlayer, playManager, eventManager, vAPI) {
