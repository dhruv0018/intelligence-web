...
Videoplayer.directive('krossoverVideoplayer', [
    ...
    function($sce) {
        ...
        function controller($scope, $state, $sce, $modal) {
            ...
            $scope.$watchGroup(['clipStartTime', 'clipEndTime'], function(clipTimes) {

                console.dir(clipTimes);
                var clipStartTime = clipTimes[0];
                var clipEndTime = clipTimes[1];

                if (clipStartTime && clipEndTime) {

                    $scope.mediaFragmentURI = '#' + clipStartTime + ',' + clipEndTime;
                }
