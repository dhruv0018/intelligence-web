...
Videoplayer.controller('Videoplayer.controller', [
    ...
    function controller($window, $rootScope, config, $scope, $state, $sce, $modal, vgFullscreen, videoplayer, videoPlayerInstance, playManager, eventManager, VG_CONFIG, VG_STATES, DEVICE) {
        ...
        document.addEventListener(vgFullscreen.onchange, onFullScreenChange(self.API));

        function onFullScreenChange(API) {

            /* If the video was previously in fullscreen before. */
            if (API.isFullScreen) {
            ...
            /* If the video was not previously in fullscreen before. */
            else {

                /* Show the video fullscreen. */
                fullScreenVideo(API.videogularElement, API.mediaElement);
            }
        }
