...
Videoplayer.controller('Videoplayer.controller', [
    'config', '$scope', '$state', '$sce', '$modal', 'VideoPlayerInstance', 'PlayManager', 'EventManager', 'VG_STATES',
    function controller(config, $scope, $state, $sce, $modal, videoPlayerInstance, playManager, eventManager, VG_STATES) {
        ...
        $scope.playerElementWidth = $scope.config.width;
        $scope.playerElementHeight = $scope.config.height;

        if ($scope.videoWidthOverride) $scope.playerElementWidth = $scope.videoWidthOverride;
        if ($scope.videoHeightOverride) $scope.playerElementHeight = $scope.videoHeightOverride;

        if ($scope.videoTitle === 'rawFilm' || $scope.videoTitle === 'indexing' || $scope.videoTitle === 'reelsPlayer') {
            ...
            if ($scope.playEndTime && currentTime > $scope.playEndTime) {
                ...
                videoPlayer.then(function(vp) {
                    ...
                    if (!playManager.playAllPlays) {
                        ...
                        var nextPlay = playManager.getNextPlay();
                        if (nextPlay) {
                            nextPlay.selectPlay();
                            nextPlay.playPlay();
                        }
