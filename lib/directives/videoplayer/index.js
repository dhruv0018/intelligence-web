...
Videoplayer.factory('VideoPlayerInstance', [
    '$q', 'VideoPlayerAPI', 'VG_EVENTS',
    function($q, videoPlayerAPI, VG_EVENTS) {
        var self = this;
        self.deferred = $q.defer();
        self.waitTime = 200;

        self.interval = setInterval(function() {
            var vAPI = videoPlayerAPI.get();
            console.log('waiting...', self.waitTime, vAPI);
            self.deferred.notify('waiting');
            if (vAPI) {
                if (vAPI.isPlayerReady()) {
                    console.log('resolve1');
                    self.deferred.resolve(vAPI);
                    clearInterval(self.interval);
                } else {
                    console.log('opr event');
                    vAPI.$on(VG_EVENTS.ON_PLAYER_READY, function(aaa) {
                        console.log('resolve2', aaa);
                        self.deferred.resolve(vAPI);
                        clearInterval(self.interval);
                    });
                }
            } else {
                self.waitTime = self.waitTime * 1.1;
            }

            if (self.waitTime > 5000) {
...
Videoplayer.factory('VideoPlayerAPI', function() {
    this.vAPI = null;
...
Videoplayer.directive('krossoverVideoplayer', [
    ...
    function($sce) {
        ...
        function link($scope, element, attributes) {
            $scope.videoElement = element.find('video');
            element.on('$destroy', function() {
                console.log('destroyel', arguments);
            });
        }
...
Videoplayer.controller('Videoplayer.controller', [
    'config', 'package', '$scope', '$state', '$sce', '$modal', 'VideoPlayerInstance', 'PlayManager', 'EventManager', 'VG_STATES', 'VideoPlayerAPI',
    function controller(config, package, $scope, $state, $sce, $modal, videoPlayer, playManager, eventManager, VG_STATES, vAPI) {
        ...
        $scope.$on('$destroy', function() {
            videoPlayer.then(function(vp) {
                //vp.pause();
                console.log('destroy=-=-=-=-=-=-');
                vp.videoElement[0].src = '';
                //angular.element(vp.videoElement[0]).remove();
            });
            playManager.clear();
            eventManager.reset();
        });
        ...
        function updateMediaFragment(newClipTime) {
            ...
            videoPlayer.then(function(vp) {
                vp.videoElement[0].src = (vp.videoElement[0].src) ? vp.videoElement[0].src.split('#')[0] + $scope.mediaFragmentURI : vp.videoElement[0].src;
            });
        }
        ...
        $scope.onPlayerReady = function(API) {
            ...
            API.videoElement.one('canplay', function() {
                console.log('canplay');
                API.videoElement = $scope.videoElement;
                console.log(API.videoElement);
                //$scope.videoElement = API.videoElement[0]; //for video buttons
                vAPI.set(API);//videoPlayer.resolve(API);
                console.log(API, vAPI.get());
            });
        };
        ...
        $scope.onUpdateTime = function(currentTime, endTime) {
            ...
            console.log('updateTime', $scope.$id);

            if ($scope.playEndTime && currentTime > $scope.playEndTime) {
                console.log('pause', $scope);
                videoPlayer.then(function(vp) {
                    vp.pause();
