...
Event.controller('Event.controller', [
    '$scope', '$q', 'ROLES', 'SessionService', 'IndexingService', 'TagsetsFactory', 'PlayersFactory', 'ScriptsService', 'EventManager', 'PlayManager', 'VideoPlayerInstance', 'VG_EVENTS', 'GamesFactory',
    function controller($scope, $q, ROLES, session, indexing, tagsets, players, scripts, eventManager, playManager, videoPlayerInstance, VG_EVENTS, games) {
        ...
        videoPlayer.then(function(vp) {
            ...
            vp.$on(VG_EVENTS.ON_UPDATE_TIME, function(eventTarget, timeParams) {
                ...
                currentVideoTime = (playManager.videoTitle === 'reelsPlayer') ? playManager.current.startTime + timeParams[0] : timeParams[0];
                if (currentVideoTime - $scope.event.time > 0) {
