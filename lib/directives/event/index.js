...
Event.controller('Event.controller', [
    ...
    function controller($scope, $q, ROLES, session, indexing, tagsets, players, scripts, eventManager, playManager) {
        ...
        $scope.items = currentUser.is(ROLES.INDEXER) ? scripts.indexerScript(tagset, $scope.event) : scripts.userScript(tagset, $scope.event);
        ...
        if (!$scope.play.possessionTeamId) {
            ...
            angular.forEach($scope.event.variableValues, function(variableValue) {
