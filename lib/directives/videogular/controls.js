        ...
        return {
            ...
            link: function(scope, elem, attr, API) {
                ...
                function seekTime(offsetX, applyToVideo) {
                    ...
                    eventManager.reset();
                    scope.currentClipTime = seekTimeVal + playStartTime;
                    scope.$apply();
                    if (applyToVideo) API.seekTime(seekTimeVal + playStartTime);
                }
    ...
    function(VG_EVENTS) {
        return {
            ...
            link: function(scope, elem, attr, API) {
                ...
                function onUpdateTime(target, params) {
                    var playStartTime = scope.playStartTime || 0;
                    var currentClipTime = scope.currentClipTime || 0;
                    var clipEndTime = scope.clipEndTime || 0;

                    percentTime = Math.round(((currentClipTime - playStartTime) / (clipEndTime - playStartTime)) * 100);
