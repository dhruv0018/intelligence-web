...
controls.directive('koTelestrationTimeBlip', [
    'VG_EVENTS', 'PlaysFactory',
    function(VG_EVENTS, plays) {
        return {
            restrict: 'E',
            require: '^videogular',
            scope: {
                telestration: '=',
                //length of the full video/clip
                duration: '='
            },
            transclude: false,
            link: function(scope, elem, attrs, API) {
                //picks up telestrations from breakdowns and reels
                var isPlayTelestration = (scope.telestration.playId) ? true : false;

                //the time of the full video for raw telestrations
                //the time of the full clip for play telestrations and reel telestrations
                var duration = 0;

                //where the blip is relative to the clip or video
                var telestrationTime = 0;

                //where the blip is placed in the scrubber
                var placementTime = 0;

                if (isPlayTelestration) {
                    var play = plays.get(scope.telestration.playId);
                    telestrationTime = scope.telestration.time - play.startTime;
                    duration = play.endTime - play.startTime;
                } else {
                    duration = scope.duration;
                    telestrationTime = scope.telestration.time;
                }

                //where the blip is placed on the scrubber
                placementTime = (telestrationTime / duration) * 100;
                elem.css('left', placementTime + '%');
            }
        };
    }
]);
