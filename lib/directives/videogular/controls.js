...
controls.directive('vgTimedisplay', [
    'VG_EVENTS',
    function(VG_EVENTS) {
        return {
            require: '^videogular',
        ...
        return {
            ...
            link: function(scope, elem, attr, API) {
                ...
                function seekTime(offsetX, applyToVideo) {
                    var seekTimeVal = offsetX / elem[0].scrollWidth * (scope.clipEndTime - scope.playStartTime);
                    eventManager.reset();
                    scope.currentClipTime = seekTimeVal + scope.playStartTime;
                    scope.$apply();
                    if (applyToVideo) API.seekTime(seekTimeVal + scope.playStartTime);
                }
    ...
    function(VG_EVENTS) {
        return {
            ...
            link: function(scope, elem, attr, API) {
                ...
                function onUpdateTime(target, params) {
                    percentTime = Math.round(((scope.currentClipTime - scope.playStartTime) / (scope.clipEndTime - scope.playStartTime)) * 100);
