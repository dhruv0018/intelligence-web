        ...
        return {
            ...
            link: {
                pre: function(scope, elem, attr, controller) {
                    ...
                    var backupScopeWidth = scope.playerWidth;
                    var backupScopeHeight = scope.playerHeight;
                    var controlsHeight = 60; //Not loaded at this point, so need to hardcode
                    var windowSizeWhenVideoStartsToCompress = 1275;
                    var windowSizeWhenVideoStopsCompressing = 1000;
                    var maxVideoWidth = 1280;
                    var minVideoWidth = 575;

                    function fitVideo() {

                        var windowWidth = $window.innerWidth;
                        if (windowWidth < windowSizeWhenVideoStartsToCompress) {
                            var newPlayerWidth = backupScopeWidth - (windowSizeWhenVideoStartsToCompress - windowWidth);
                            scope.playerWidth = Math.max(newPlayerWidth, minVideoWidth);
                            scope.playerHeight = (scope.playerWidth / 16 * 9) + controlsHeight;
                        } else {
                            scope.playerWidth = backupScopeWidth;
                            scope.playerHeight = backupScopeHeight;
                        }
                        scope.$apply();
                    }
                    ...
                    angular.element($window).bind('resize', fitVideo);
