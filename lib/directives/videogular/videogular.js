        ...
        return {
            ...
            controller: ['$scope', '$filter', function($scope, $filter) {
                ...
                this.setTimeDisplay = function setTimeDisplay(timeInSeconds) {
                    currentTimeElement = currentTimeElement.length ? currentTimeElement : angular.element(document.getElementById('videoPlayerCurrentTime'));

                    if (!currentTimeElement.length) throw new Error('element with id \'videoPlayerCurrentTime\' not found');

                    /*
                     *  REQUIRES STYLE:
                     *    #videoPlayerCurrentTime:after {
                     *      content: attr(data-current-time-value);
                     *    }
                     */
                    currentTimeElement.attr('data-current-time-value', $filter('secondsToTime')(timeInSeconds));
                };
                ...
                $scope.updateSize = function() {
                    if (isPlayerReady) {
                        ...
                        if (angular.element($window)[0].fullScreenAPI && angular.element($window)[0].fullScreenAPI.isFullScreen() || isFullScreen) {
                            ...
                            self.elementScope.css('width', parseInt($scope.playerWidth, 10) + 'px');
                            self.elementScope.css('height', parseInt($scope.playerHeight, 10) + 'px');
            ...
            link: {
                pre: function(scope, elem, attr, controller) {
                    ...
                    controller.scrubbarElement = controller.elementScope.find('data-vg-scrubbar');
                    controller.controlsElement = controller.elementScope.find('data-vg-controls');
