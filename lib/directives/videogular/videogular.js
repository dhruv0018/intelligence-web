...
videogular.directive('videogular', [
    '$rootScope', '$window', 'VG_STATES', 'VG_EVENTS', 'VG_UTILS', 'VG_CONFIG', 'VideoElement', 'PlayManager',
    function($rootScope, $window, VG_STATES, VG_EVENTS, VG_UTILS, VG_CONFIG, videoElementService, playManager) {
        ...
        return {
            ...
            controller: ['$scope', '$element', '$filter', function($scope, $element, $filter) {
                ...
                this.seekTime = function(value, offset, byPercent) {
                    ...
                    if (playManager.videoTitle === 'reelsPlayer' && !isNaN(offset)) {
                        second = value - offset;
                    }
            ...
            link: {
                pre: function(scope, elem, attr, controller) {
                    ...
                    if (scope.isMobile) {
                        VG_CONFIG.CONTROLS_HEIGHT = 0;
                        controller.videoElement[0].controls = true;
                    }
                    ...
                    scope.fitVideo = function fitVideo() {
                        ...
                        if (angular.element($window)[0].fullScreenAPI && angular.element($window)[0].fullScreenAPI.isFullScreen()) {
                            ...
                            elem.css('height', ($window.innerHeight - VG_CONFIG.CONTROLS_HEIGHT) + 'px');
                            controller.videoElement.css('width', $window.innerWidth + 'px');
                            controller.videoElement.css('height', ($window.innerHeight - VG_CONFIG.CONTROLS_HEIGHT) + 'px');
                            controller.videoElement.css('max-width', '100%');
                            controller.videoElement.css('max-height', '100%');
                        } else {
                            // Aspect ratio is calculated before adding the height required for the controls
                            var videoRect = elem.parent()[0].getBoundingClientRect();
                            var currentVideoWidth = videoRect.width;
                            var currentVideoHeight = videoRect.height;

                            var parentWithHeight = getParentWithHeight(elem);
                            ...
                            if (!parentWithHeight) {
                                ...
                                * only fit the player within the parent's height if on desktop */
                                ...
                                elem.css('width', properWidth + 'px');
                                elem.css('height', properPlayerHeight + 'px');
                                controller.videoElement.css('max-width', properWidth + 'px');
                                controller.videoElement.css('max-height', properPlayerHeight + 'px');
                            }
                        }
                    };
