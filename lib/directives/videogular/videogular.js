...
videogular.factory('VideoElement', function() {
    ...
    this.videoElement.setAttribute('data-vg-src', new Date().getTime());
    ...
    return {
        setSources: function(sources) {
            ...
            console.log('heres1', self.videoElement.canPlayType, sources);
            if (self.videoElement.canPlayType) {
                for (var i = 0, l = sources.length; i < l; i++) {
                    canPlay = self.videoElement.canPlayType(sources[i].type);

                    if (canPlay == 'maybe' || canPlay == 'probably') {
                        angular.element(self.videoElement).attr('src', sources[i].src);
                        angular.element(self.videoElement).attr('type', sources[i].type);
                        break;
                    }
                }
            }
            // It's a crappy browser and it doesn't deserve any respect
                ...
                angular.element(self.videoElement).attr('src', sources[0].src);
                angular.element(self.videoElement).attr('type', sources[0].type);
            }
        ...
        get: function() {
            console.dir('t', this, self);
            return self.videoElement;
        },
        getWrapped: function() {
            console.log('t', this, self);
            return angular.element(self.videoElement);
        }
    };
        ...
        return {
            ...
            controller: ['$scope', function($scope) {
                ...
                $scope.onPlayerReady = function() {
                    self.videoElement[0].addEventListener('loadedmetadata', $scope.onLoadedMetaData);
                ...
                $scope.onUpdateTime = function(event) {
                    console.log('^updateTime', $scope.$id, $scope);
            ...
            link: {
                pre: function(scope, elem, attr, controller) {
                    ...
                    console.log('controller.videoElement', controller.videoElement, videoElementService);
                    console.log(scope.sources);
                    console.dir(controller.videoElement);
                    console.dir(videoElementService);
                    ...
                    console.log('find');
                    console.log(controller.elementScope.find('#krossoverVideoPlayer').length);
                    console.dir(controller.elementScope.find('#krossoverVideoPlayer'));
                    if (!controller.elementScope.find('#krossoverVideoPlayer').length) {
                        controller.elementScope.append(controller.videoElement);
                    }
                }
            }
        };
            ...
            link: {
                pre: function(scope, elem, attr) {
                    ...
                    function changeSource() {
                        ...
                        // It's a cool browser
                        console.log('here1', element[0].canPlayType, sources);
