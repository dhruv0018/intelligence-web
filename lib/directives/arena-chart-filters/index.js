...
function ArenaChartFiltersDirective(
    ...
    function ArenaChartFiltersLink(scope, element, attributes) {
        ...
        scope.teamPlayerList = teamPlayerList.map((player) => {
            ...
            let jerseyNumber = player.getJerseyNumber(team.roster);
            jerseyNumber = $filter('padSpacesToFixedLength')(jerseyNumber, 3, 'left');

            let positionNames;

            try {
                positionNames = positionset.getPositionNames(team.roster.playerInfo[player.id].positionIds);
        ...
        scope.opposingTeamPlayerList = opposingTeamPlayerList.map((player) => {
            ...
            let jerseyNumber = player.getJerseyNumber(opposingTeam.roster);
            jerseyNumber = $filter('padSpacesToFixedLength')(jerseyNumber, 3, 'left');

            let positionNames;

            try {
                positionNames = positionset.getPositionNames(opposingTeam.roster.playerInfo[player.id].positionIds);
                positionNames = '(' + positionNames.join(', ') + ')';
            } catch (error) {
                positionNames = '';
            }
