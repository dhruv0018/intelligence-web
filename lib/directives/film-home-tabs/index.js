...
//                    },
                    regular: {
                        items: gamesList.filter(function(game) {
                            return game.gameType === GAME_TYPES.CONFERENCE.id && !game.isSharedWithUser(scope.user) || game.gameType === GAME_TYPES.NON_CONFERENCE.id;
                        }),
                        selected: false,
                        description: 'Regular'
                    },
                    breakdown: {
                        items: gamesList.filter(function(game) {
                            return game.isDelivered() && !game.isSharedWithUser(scope.user);
                        }),
                        selected: false,
                    ...
                    scouting: {
                        items: gamesList.filter(function(game) {
                            return game.gameType === GAME_TYPES.SCOUTING.id && !game.isSharedWithUser(scope.user);
                        }),
                        selected: false,
                    ...
                    scrimmage: {
                        items: gamesList.filter(function(game) {
                            return game.gameType === GAME_TYPES.SCRIMMAGE.id && !game.isSharedWithUser(scope.user);
                        }),
                        selected: false,
                    ...
                    shared: {
                        items: gamesList.filter(function(game) {
                            return game.isSharedWithUser(scope.user);
                        }),
                        selected: false,
