        ...
        function link(scope) {
            ...
            userRoles.forEach(function(role, index) {
                ...
                Object.keys(teamPlayers).forEach(function(teamPlayerId) {
                    ...
                    let playerIds = userPlayers.map(function(player) {return player.id;});

                    if (~playerIds.indexOf(Number(teamPlayerId))) {

                        let thisPlayer = players.get(teamPlayerId);

                        if (thisPlayer.userId == session.getCurrentUserId()) {
                ...
                let positions = positionIds.map(positionId => positionset.getPositionById(positionId));
