    ...
    function directive(users, players, $http, teams, positionsets, leagues, sports, session, ROLE_TYPE, config, alerts) {
        ...
        function link(scope) {
            ...
            for (var index = 0; index < scope.players.length; index++) {
                ...
                if (~tempPlayer.rosterIds.indexOf(scope.team.roster.id)) {
                    scope.roleInfo = {
                        team: scope.team,
                        positionIds: tempPlayer.positionIds[scope.team.roster.id],
                        jerseyNumber: tempPlayer.jerseyNumbers[scope.team.roster.id],
