/* Fetch angular from the browser scope */
var angular = window.angular;

/**
 * Teams page module.
 * @module Teams
 */
var Teams = angular.module('teams', [
    'ui.router',
    'ui.bootstrap'
]);

/* Cache the template file */
Teams.run([
    '$templateCache',
    function run($templateCache) {

        $templateCache.put('team.html', require('./team.html'));
        $templateCache.put('teams.html', require('./teams.html'));
    }
]);

/**
 * Teams page state router.
 * @module Teams
 * @type {UI-Router}
 */
Teams.config([
    '$stateProvider',
    function config($stateProvider) {

        $stateProvider

            .state('teams', {
                url: '/teams',
                views: {
                    'header': {
                        templateUrl: 'header.html',
                        controller: 'HeaderController'
                    },
                    'main': {
                        templateUrl: 'teams.html',
                        controller: 'TeamsController'
                    }
                }
            })

            .state('team', {
                url: '/teams',
                views: {
                    'header': {
                        templateUrl: 'header.html',
                        controller: 'HeaderController'
                    },
                    'main': {
                        templateUrl: 'team.html',
                        controller: 'TeamController'
                    }
                }
            });
    }
]);

/**
 * Team value. Holds data for the current team.
 * @module Teams
 * @name TeamsValue
 * @type {Value}
 */
Teams.value('TeamValue', {

    data: null,

    set: function(team) {
        this.data = team;
    }
});

/**
 * Team controller. Controls the view for adding and editing a single team.
 * @module Teams
 * @name TeamController
 * @type {Controller}
 */
Teams.controller('TeamController', [
    '$rootScope', '$scope', '$state', 'UsersResource', 'TeamValue', 'TeamsFactory', 'SchoolsResource',
    function controller($rootScope, $scope, $state, users, team, teams, schools) {

        $scope.team = team.data;

        if ($scope.team && $scope.team.roles) {

            for (var i = 0; i < $scope.team.roles.length; i++) {

                var userId = $scope.team.roles[i].userId;
                $scope.team.roles[i].user = users.get({ id: userId });
            }
        }

        $scope.schools = schools.query();

        $scope.onlyCurrentRoles = function(role) {

            /* Assume falsy value means the tenure end date hasn't been set yet. */
            if (!role.tenureEnd) return true;

            var tenureEnd = new Date(role.tenureEnd);

            /* Assume invalid date means the tenure end date hasn't been set yet. */
            if (isNaN(tenureEnd.getTime())) return true;

            return false;
        };

        $scope.onlyPastRoles = function(role) {

            /* Assume falsy value means the tenure end date hasn't been set yet. */
            if (!role.tenureEnd) return false;

            var tenureEnd = new Date(role.tenureEnd);

            /* Assume invalid date means the tenure end date hasn't been set yet. */
            if (isNaN(tenureEnd.getTime())) return false;

            var today = new Date();

            return today - tenureEnd > 0;
        };

        $scope.next = function() {

            var team = $scope.team;

            if (!team) return;

            team.schoolId = team.school.id;

            teams.save(team);

            $state.go('teams');
        };

        $scope.cancel = function() {

            $state.go('teams');
        };
    }
]);

/**
 * Teams controller. Controls the view for displaying multiple teams.
 * @module Teams
 * @name TeamsController
 * @type {Controller}
 */
Teams.controller('TeamsController', [
    '$rootScope', '$scope', '$state', 'TeamValue', 'TeamsFactory',
    function controller($rootScope, $scope, $state, team, teams) {

        $scope.teams = teams.getAll();

        $scope.add = function() {

            team.set(null);
            $state.go('team');
        };

        $scope.edit = function(data) {

            team.set(data);
            $state.go('team');
        };
    }
]);

