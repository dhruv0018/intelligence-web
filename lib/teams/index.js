...
Teams.controller('TeamController', [
    '$rootScope', '$scope', '$state', '$stateParams', 'UsersResource', 'TeamValue', 'TeamsFactory', 'SportsResource', 'LeaguesResource', 'SchoolsResource',
    function controller($rootScope, $scope, $state, $stateParams, users, team, teams, sports, leagues, schools) {

        if ($stateParams.id) {

            $scope.team = teams.get($stateParams.id);

        } else {

            $scope.team = team.data || { name: 'New Team' };
        }

        if ($scope.team && $scope.team.roles) {

            for (var i = 0; i < $scope.team.roles.length; i++) {

                var userId = $scope.team.roles[i].userId;
                $scope.team.roles[i].user = users.get({ id: userId });
            }
        }
