...
QaPickup.controller('QaPickup.controller', [
    '$scope', '$state', '$window', '$modalInstance', 'Utilities', 'GamesFactory', 'game', 'TeamsFactory', 'LeaguesFactory', 'SportsFactory', 'SessionService', 'HOURS',
    function controller($scope, $state, $window, $modalInstance, utilities, games, game, teams, leagues, sports, session, HOURS) {
            ...
            games.fetch(gameId).then(function(updatedGame) {
                ...
                if(currentAssignment.isQa) {
                    ...
                    if($scope.game.timeRemaining > 0) {
                        ...
                        deadline = now.add(HOURS.QA_PICKUP.DEADLINE_PASSED, 'hours').format();
                    }
