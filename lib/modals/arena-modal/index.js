...
ArenaModal.controller('ArenaModal.controller', [
    ...
    function controller($scope, $state, $modalInstance) {
        ...
        $scope.$watch('coordinates', function watchCoordinates(coordinates) {
            ...
            $scope.$parent.event.variableValues[$scope.item.id].value = $scope.$parent.event.variableValues[$scope.item.id].value || {};

            $scope.$parent.event.variableValues[$scope.item.id].value.coordinates = coordinates;
        });
        ...
        $scope.$watch('region.id', function watchRegionId(regionId) {
            ...
            $scope.$parent.event.variableValues[$scope.item.id].value.region = regionId;
        });
    }
]);
