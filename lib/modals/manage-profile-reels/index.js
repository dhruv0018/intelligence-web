...
ManageProfileReels.controller('ManageProfileReels.controller', [
    ...
    function controller($scope,
        ...
        function updateProfileReels() {
            $scope.profileReels = reels.getSortedProfileReels($scope.user);
        ...
        $scope.reorderProfileReels = function() {
            ...
            $scope.profileReels.forEach( (reel, index) => {
                $scope.user.profile.reelIds.forEach(reelId => {
                    if (reel.id === reelId) {
                        sortedProfileReelIds[index] = reelId;
                    }
                });
            });
            $scope.user.profile.reelIds = sortedProfileReelIds;
            $scope.reordering = false;
        };

        $scope.saveProfile = function() {
            $scope.user.save().then( () => $modalInstance.close());
        };
    }
]);
