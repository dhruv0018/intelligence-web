...
ShareFilm.controller('ShareFilm.controller', [
    '$scope', '$state', '$modalInstance', '$timeout', 'config', 'SessionService', 'ShareFilm.Modal', 'BasicModals', 'Film', 'GamesFactory', 'ReelsFactory', 'UsersFactory', 'ROLES',
    function controller($scope, $state, $modalInstance, $timeout, config, session, modal, basicModal, film, games, reels, users, ROLES) {
        ...
        $scope.embedSize = 'width:480;height:500px;';
        $scope.film = film;
        ...
        $scope.shareReelWithTeamToggle = function() {
            ...
            $scope.film.save();
            $scope.sharedWithTeam = !$scope.sharedWithTeam;
        };
