...
ShareFilm.controller('ShareFilm.controller', [
    '$scope', '$state', '$modalInstance', '$timeout', 'config', 'UsersStorage', 'TeamsStorage', 'SchoolsStorage', 'SessionService', 'ShareFilm.Modal', 'BasicModals', 'Film', 'GamesFactory', 'ReelsFactory', 'UsersFactory', 'TeamsFactory', 'PlayersFactory', 'ROLES', 'ROLE_TYPE',
    function controller($scope, $state, $modalInstance, $timeout, config, usersStorage, teamsStorage, schoolsStorage, session, modal, basicModal, film, games, reels, users, teams, players, ROLES, ROLE_TYPE) {
        ...
        $scope.sharingBreakdownOptions = [
            {value: false, label: 'Sharing raw film only'},
            {value: true, label: 'Sharing raw film and breakdown'}
        ];
        ...
        $scope.shareSubmit = function(user) {
            ...
            if (!teamsStorage.isStored(team)) {
                teamsStorage.set(team);
            }

            if (!schoolsStorage.isStored(school)) {
        ...
        //TODO: There are lot of complex validations in the templete. add the validators in the scope and use them in template 
