...
ShareFilm.controller('ShareFilm.controller', [
    ...
    function controller(
        ...
        // Get film exchanges user's team is a part of
        if($scope.currentUser.is(ROLES.COACH) && $scope.isGame) {
            teams.getFilmExchanges(currentRole.teamId).then(filmExchanges => {
                filmExchanges.map(filmExchange => {
        ...
        $scope.shareGamesWithFilmExchange = function() {
            ...
            $scope.filmExchanges.forEach(filmExchange => {
                if (filmExchange.gameIsShared) {
                    gameForFilmExchange.sportsAssociation = filmExchange.sportsAssociation;
                    gameForFilmExchange.conference = filmExchange.conference;
                    gameForFilmExchange.gender = filmExchange.gender;
                    gameForFilmExchange.sportId = filmExchange.sportId;
                    filmExchangeFactory.shareGameWithFilmExchange(gameForFilmExchange);
                }
            });
        };
